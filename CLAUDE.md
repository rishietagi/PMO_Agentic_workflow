...
| Embeddings | **BAAI/bge-base-en-v1.5** via `sentence-transformers`, local | Free, no rate limits, no network dependency at query time. Runs on **GPU when available** (CUDA torch; `config.resolve_device()`), else CPU. NOTE: the dev GPU is a 4 GB RTX 3050 — big enough for embeddings/reranking (fast) but **too small for marker's multi-model OCR**, so OCR stays on CPU (set `PMO_DEVICE`/`TORCH_DEVICE` to override). |
...
| Reranking | **cross-encoder/ms-marco-MiniLM-L-6-v2** via `sentence-transformers`, local | Free, fast on CPU for reranking a top-20 shortlist. |
